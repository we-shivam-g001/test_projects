...
let model_namespace = 'VaahCms\\Modules\\Project\\Models\\Contact';


let base_url = document.getElementsByTagName('base')[0].getAttribute("href");
let ajax_url = base_url + "/project/contacts";
...
export const useContactStore = defineStore({
    ...
    actions: {
        ...
        async getItemAfter(data, res)
        {
            ...
            {
                this.item = data;
                this.item.companies=data.company;
                this.item.added_by_user=data.responsible_user.name;
            }else{
                this.$router.push({name: 'contacts.index',query:this.query});
            }
        ...
        async itemAction(type, item=null){
            ...
            item.expires_at = `${item.month}/${item.expiry_year}`;
        ...
        async formActionAfter (data)
        {
            switch (this.form.action)
            {
                ...
                case 'create-and-clone':
                    this.item.id = null;
                    this.$router.push({name: 'contacts.form',query:this.query,params: { id: null }});
                    await this.getFormMenu();
                    break;
                case 'trash':
                case 'restore':
        ...
        getFormInputsAfter: function (data, res) {
            if(data)
            {
                this.item.added_by_user = data.fill.responsible_user;
                let self = this;
                Object.keys(data.fill).forEach(function(key) {
                    if (key !== 'added_by_user') {
        ...
        toForm()
        {
            this.item = vaah().clone(this.assets.empty_item);
            this.getFormMenu();
            this.$router.push({name: 'contacts.form',query:this.query})
        },
        //---------------------------------------------------------------------
        toView(item)
        {
            ...
            this.$router.push({name: 'contacts.view', params:{id:item.id},query:this.query})
        },
        //---------------------------------------------------------------------
        toEdit(item)
        {
            ...
            this.$router.push({name: 'contacts.form', params:{id:item.id},query:this.query})
        },
        //---------------------------------------------------------------------
        isViewLarge()
        {
        ...
        async searchCompanies(event) {
            ...
            await vaah().ajax(
                this.ajax_url+'/search/companies',
                this.searchCompaniesAfter,
                options
            );
        },
        //---------------------------------------------------------------------
        searchCompaniesAfter(data,res){
            if(data){
                this.company_suggestions = data;
            }
        },
        ...
        setCcompany(event) {
            if (this.item_company) {
                this.item_company.vh_company_id = (event && event.value && event.value.id) ? toRaw(event.value).id : null;
            }
        },
        ...
        {
            this.item_company = vaah().clone(this.assets.empty_item_company);
        ...
        async saveCompany(type,item){
            ...
            await vaah().ajax(
                this.ajax_url+'/create/new-company',
                this.saveCompanyAfter,
                options
            );
        },
        ...
        saveCompanyAfter(data){
             if (data){
                 this.item.companies=data;
             }
        ...
        async saveExistingCompany(type,item){
            type ['contact_id'] = this.contact_id
            if(this.route.params)
            {
                this.item_company.company_id = this.route.params.id
            }
