...
let model_namespace = 'VaahCms\\Modules\\Project\\Models\\Company';


let base_url = document.getElementsByTagName('base')[0].getAttribute("href");
let ajax_url = base_url + "/project/companies";
...
export const useCompanyStore = defineStore({
    ...
    state: () => ({
        ...
        countries:null,
        contact_suggestions:null,
        show_contact_panel:false,
        months: Array.from({ length: 12 }, (_, index) => ({ name: index + 1, code: index + 1 })),
    ...
    actions: {
        ...
        async getItemAfter(data, res)
        {
            if(data)
            {
                ...
                this.$router.push({name: 'companies.index',query:this.query});
            }
        ...
        async formActionAfter (data)
        {
            switch (this.form.action)
            {
                ...
                case 'create-and-clone':
                    this.item.id = null;
                    this.$router.push({name: 'companies.form',query:this.query,params: { id: null }});
                    await this.getFormMenu();
                    break;
                case 'trash':
                case 'restore':
        ...
        getFormInputsAfter: function (data, res) {
            if(data)
            {
                this.item.contacts=data.fill.contacts;
                let self = this;
        ...
        toForm()
        {
            this.item = vaah().clone(this.assets.empty_item);
            this.getFormMenu();
            this.$router.push({name: 'companies.form',query:this.query})
        },
        //---------------------------------------------------------------------
        toView(item)
        {
            ...
            this.$router.push({name: 'companies.view', params:{id:item.id},query:this.query})
        },
        //---------------------------------------------------------------------
        toEdit(item)
        {
            ...
            this.$router.push({name: 'companies.form', params:{id:item.id},query:this.query})
        },
        //---------------------------------------------------------------------
        isViewLarge()
        {
        ...
        async searchContacts(event) {
            ...
            await vaah().ajax(
                this.ajax_url+'/search/contacts',
                this.searchContactsAfter,
                options
            );
        },
        //---------------------------------------------------------------------
        searchContactsAfter(data,res){
            if(data){
                this.contact_suggestions = data;
            }
        },
        ...
        setContact(event) {
            // this.item.vh_contact_id = (event && event.value && event.value.id) ? toRaw(event.value).id : null;
            if (this.item_contact) {
                this.item_contact.vh_contact_id = (event && event.value && event.value.id) ? toRaw(event.value).id : null;
            }
        },

        //---------------------------------------------------------------------

        async toViewContacts(item)
        {
            this.show_contact_panel = true;
            this.company_id=item.id;
            this.company_name=item.name;

            if (item.id) {
                await vaah().ajax(
                    ajax_url + '/get-contact-list'+'/' + item.id,
                    this.toViewContactsAfter
                );
            }
        },
        ...
        async toContactCreate(item)
        {
            this.item_contact = vaah().clone(this.assets.empty_item_contact);
            this.company_id=item.id;
            this.show_create_contact = true;
        },
        //---------------------------------------------------------------------
...
//---------------------------------------------------------------------
        ...
        async createContact(type,item){
            ...
            await vaah().ajax(
                this.ajax_url+'/create/new-contact',
                this.createContactAfter,
                options
            );
        },
        ...
        async saveExistingContact(type,item){

            if (!type ) {
                vaah().toastErrors(['Select a contact']);
                return;
            }
            type ['company_id'] = this.company_id
            if(this.route.params)
            {
                this.item_contact.company_id = this.route.params.id
            }
            ...
            await vaah().ajax(
                this.ajax_url+'/save/with-existing-company',
                this.saveExistingContactAfter,
                options
            );
        },
        ...
        saveExistingContactAfter(){
            this.getList();
            this.show_create_contact = false;
        },
