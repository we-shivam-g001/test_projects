...
class Company extends VaahModel
{
    ...
    protected $fillable = [
        'uuid',
        'name',
        'slug','email','phone','fax_number','zip_code','city','street','country','suit',
        'is_active',
        'created_by',
        'updated_by',
        'deleted_by',
    ...
    //-------------------------------------------------
    public function companiesContacts()
    {
        return $this->belongsToMany(Contact::class, 'vh_contacts', 'vh_company_id', 'vh_contact_id');
    }
    ...
    {
        return $this->hasMany(Contact::class, 'vh_company_id', 'id');
    ...
    public static function createItem($request)
    {
        ...
        if ($email) {
            $response['success'] = false;
            $response['errors'][] = 'This company email already exist';
            return $response;
        }
        // check if slug exist
        ...
        $contact_id = $inputs['contacts']['id'] ?? null;

        if ($contact_id) {
            $contact = Contact::find($contact_id);
            if ($contact) {
                $contact->vh_company_id = $item->id;
                $contact->save();
            }
        }
    ...
    public static function createCompany($request)
    {
        ...
        $contactId = isset($inputs['contacts']['id']) ? $inputs['contacts']['id'] : $inputs['contact_id'];
    ...
    {
        $list = self::getSorted($request->filter)->with('contacts');
    ...
    public static function getItem($id)
    {
        ...
        $item = self::where('id', $id)
            ->with(['createdByUser', 'updatedByUser', 'deletedByUser','contacts'])
            ->withTrashed()
            ->first();
    ...
    public static function seedSampleItems($records=100)
    {
        ...
        while($i < $records)
        {
            ...
            if (isset($inputs['contacts']) && $inputs['contacts']) {
                $contact_id = $inputs['contacts']['id'];
    ...
    //-------------------------------------------------
    public static function searchContacts($request){
        $active_customers = Contact::select('id', 'first_name', 'last_name')
    ...
    //-------------------------------------------------
    public static function getContactsList($id)
    {
//        dd($id);
